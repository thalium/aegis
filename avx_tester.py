...
VECTOR_PREFIX_WIDTHS: dict[str, int] = {
    "xmm": 16,
    "ymm": 32,
    "zmm": 64,
}
...
AVX_WHERE = (
    "instruction_id IN (SELECT instruction_id FROM instruction_tags WHERE tag = 'avx')"
)
...
def iter_test_specs_from_db(dsn: str) -> Iterator[dict]:
    with open_db(dsn) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT test_case_id, state_index FROM test_results "
                f"WHERE test_case_id IN (SELECT id FROM test_cases WHERE {AVX_WHERE})"
            )
...
def serialize_state(cpu_state: PyCpuState, reg_keys: list[str]) -> dict[str, object]:
...
def main():
    ...
    with db_con.cursor() as cur:
        cur.execute(
            f"""
            SELECT
                (SELECT SUM(jsonb_array_length(initial_states)) FROM test_cases WHERE {AVX_WHERE}) -
                (SELECT COUNT(*) FROM test_results WHERE test_case_id IN (SELECT id FROM test_cases WHERE {AVX_WHERE}))
        """
        )
